...
class NaturalAdagradNGDOptimizer:
    ...
    def approx_adaptive_update_op(self, g_step):
        cumulative_average_ops = [tf.assign(lv, (tf.cast(g_step, tf.float32) * lv + v) / (tf.cast(g_step, tf.float32) + 1)) for lv, v in zip(self.lagged_vars, self.vars)]
        approx_adap_op = tf.cond(g_step > 0, lambda: cumulative_average_ops, lambda: self.lagged_vars)
    ...
    def minimize(self,
        ...
        maybe_assign_grads_to_cg_prev = tf.cond(tf.equal(global_step, 0), lambda: self.assign_grads_to_prev(grads), lambda: grads)
            ...
            with tf.control_dependencies(approx_adap_ops):
                with tf.control_dependencies(set_vars_to_tmp):
                    with tf.control_dependencies(set_lagged_to_vars):
                        ...
                        cg_op = cg.cg_solve(*cg_inputs, bias2_correction=1)
