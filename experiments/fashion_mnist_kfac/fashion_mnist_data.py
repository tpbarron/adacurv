...
def load_mnist_as_tensors(train=True, flatten_images=True):
...
#   return images, labels, num_examples
  ...
  train_dat, test_dat = tf.keras.datasets.fashion_mnist.load_data()
  if train:
    (images, labels) = train_dat
  else:
    (images, labels) = test_dat
...
def load_mnist_as_dataset(train=True, flatten_images=True):
  ...
  """
  images, labels, num_examples = load_mnist_as_tensors(train=train,
...
def load_mnist_as_iterator(num_epochs, batch_size,
  ...
  if use_fake_data:
    ...
    dataset, num_examples = load_mnist_as_dataset(train=train, flatten_images=flatten_images)

  dataset = (dataset.shuffle(num_examples).repeat(num_epochs)
             .batch(batch_size).prefetch(5))
  return tf.compat.v1.data.make_one_shot_iterator(dataset).string_handle(), dataset.output_types, dataset.output_shapes #.get_next()
