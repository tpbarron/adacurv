...
def test(model):
    ...
    vec_val1 = np.random.random(size=(1, 105))
    vec_val2 = np.random.random(size=(1, 12))
    inp = Variable(torch.FloatTensor([inp_val]), requires_grad=True)
    v1 = Variable(torch.FloatTensor([vec_val1]), requires_grad=False)
    v2 = Variable(torch.FloatTensor([vec_val2]), requires_grad=False)
    z = model(inp)
    ...
    print (len(params))
    input("")
    gnvp_roplop = GNvp_RopLop(l, z, list(model.parameters())[0:2], v1)
    print (gnvp_roplop)

    gnvp_roplop = GNvp_RopLop(l, z, list(model.parameters())[2:], v2)
    print (gnvp_roplop)
