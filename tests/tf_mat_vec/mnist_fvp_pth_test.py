...
def test(model):
    ...
    inp_val = np.random.random(size=(128, 784))
    inp = Variable(torch.FloatTensor(inp_val), requires_grad=True)

    z = model(inp)
    tar = torch.empty(128, dtype=torch.long).random_(10)
    f = F.cross_entropy(z, tar)

    import time
    n = 100
    s = time.time()
    for i in range(n):
        vec_val = np.random.random(size=(407050,))
        v = Variable(torch.FloatTensor(vec_val), requires_grad=False)
        # gnvp_roplop = GNvp_RopLop(f, z, list(model.parameters()), v)
        fvp = Hvp_dbl_bp(f, list(model.parameters()), v)
    e = time.time()
